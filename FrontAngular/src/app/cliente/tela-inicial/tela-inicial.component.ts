...
export class TelaInicialComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http.get<Cliente>("http://localhost:8080/clientes/" + id).subscribe((dados) => this.cliente = dados);
    this.carregarDados();
  }

  carregarDados() {
    this.pedidoService.getAbertosByUser(this.cliente.idCliente).subscribe((pedidos) => {
